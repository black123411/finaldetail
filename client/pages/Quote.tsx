...
export default function Quote() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <section className="bg-slate-950 py-20 text-white"><div className="container mx-auto px-4"><div className="max-w-3xl"><p className="text-sm font-black uppercase tracking-[.18em] text-blue-300">Contact me</p><h1 className="mt-4 text-5xl font-black tracking-tight md:text-6xl">Tell me about your vehicle.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">The fastest way to get an answer is to text me a few clear photos. If you would rather use the form, send the basics and I will look it over myself.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href={textHref} onClick={() => trackEvent('click_text_quote', { location: 'quote_hero' })} className="inline-flex min-h-14 items-center justify-center gap-2 bg-blue-600 px-7 font-black text-white hover:bg-blue-700"><MessageSquare className="h-5 w-5" /> Text Me Photos</a><a href="tel:7123056313" onClick={() => trackEvent('click_call', { location: 'quote_hero' })} className="inline-flex min-h-14 items-center justify-center gap-2 border border-slate-600 px-7 font-black text-white hover:border-blue-400"><Phone className="h-5 w-5" /> Call (712) 305-6313</a></div></div></div></section>

      <section className="py-16 md:py-24"><div className="container mx-auto grid gap-12 px-4 lg:grid-cols-[.7fr_1.3fr]"><aside><h2 className="text-3xl font-black tracking-tight">Send a contact request</h2><p className="mt-4 leading-7 text-slate-600">Tell me what you drive, what is bothering you about it, and what you want cleaned, corrected, or protected.</p><div className="mt-8 border-l-4 border-blue-600 bg-white p-6"><h3 className="font-black">Sending photos?</h3><p className="mt-2 text-sm leading-6 text-slate-600">Photos are not uploaded through this form. Use the text button so I can see them clearly on my phone.</p></div></aside>
        ...
        </div>{error && <div role="alert" className="mt-6 border-l-4 border-red-600 bg-red-50 p-4 text-sm font-semibold text-red-800">{error}</div>}<button type="submit" disabled={submitting} className="mt-7 inline-flex min-h-14 w-full items-center justify-center gap-2 bg-blue-600 px-7 font-black text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{submitting ? <><Loader2 className="h-5 w-5 animate-spin" /> Sending...</> : <>Send Request <ArrowRight className="h-5 w-5" /></>}</button><p className="mt-4 text-sm text-slate-500">I will look it over before I recommend a service or quote anything that depends on condition.</p></form>
      </div></section>
    </div>
  );
}

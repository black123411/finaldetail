            ...
            {popular.map(({ service, label, fallback }) => (
              <article key={service.id} className="flex min-h-72 flex-col border-b border-r border-slate-300 bg-white p-7 lg:p-9">
                ...
                <div className="mt-7 flex flex-wrap gap-2">
                  ...
                  <a href={getSquareBookingLink(service.id)} onClick={(event) => trackBookingHandoff(event, { location: 'home_popular', service_id: service.id })} className="inline-flex min-h-11 items-center bg-blue-600 px-5 font-black text-white hover:bg-blue-700">Book</a>
                </div>
              </article>
            ))}
          </div>

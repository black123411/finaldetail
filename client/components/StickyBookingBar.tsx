...
export default function StickyBookingBar() {
  ...
  const target = isInquiryOnly ? '/quote' : serviceId ? getSquareBookingLink(serviceId) : BOOKING_LINK;
  const label = isInquiryOnly ? 'Request Quote' : serviceId ? 'Book This Service' : 'Book Now';
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-800 bg-slate-950 p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] md:hidden">
      <div className="mx-auto flex max-w-lg gap-2">
        ...
        {isInquiryOnly ? (
          <Link to={target} onClick={() => trackEvent('begin_quote', { location: 'sticky_mobile_bar', service_id: serviceId })} className="flex min-h-12 flex-1 items-center justify-center gap-2 bg-blue-600 px-3 text-center text-sm font-black text-white">
